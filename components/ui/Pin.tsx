...
export const PinPerspective = ({
  ...
  const AnimatedDiv = ({ delay = 0, ...props }) => (
    <motion.div
      initial={{
        opacity: 0,
        scale: 0,
    ...
    <motion.div className="pointer-events-none w-full h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      <div className="w-full h-full -mt-7 flex-none inset-0">
        ...
        <div className="absolute right-1/2 bottom-1/2">
          ...
          <motion.div className="translate-x-[1.5px] bg-cyan-600 translate-y-[14px] w-[4px] h-[4px] rounded-full z-40 blur-[3px]" />
          <motion.div className="translate-x-[0.5px] bg-cyan-300 translate-y-[14px] w-[2px] h-[2px] rounded-full z-40" />
        </div>
      </div>
    </motion.div>
  );
